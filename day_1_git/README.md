...
```
1. Initialize git : goto your working directory and initialize git
```
$ git init
```

1. You probably can see a new directory called **.git** Ignore it now.

1. Now we have to check the status of initializing git by typing
```
$ git status
```
files are now on working directory, Very 1st stage on git.
1. Staging area : It's time to transfer all file to staging area.
...
$ git add .
```
1. you can again check the status, to see 'whats happend'
```
$ git status
```
your file is now on staging area. :)
1.Commit: Final stage of git. To commit staging section just typing
```
...
```
1. Now add and commit all again.
```
$ git add .
...
$ git log
```
1. you probably see something like this
...
1. Commits are called as saved snapshots of your code. Checking out an old commit is a read-only operation. So, it’s impossible to harm your repository while viewing an old revision.

1. During the normal course of development, the HEAD usually points to master or some other local branch, but when you check out a previous commit, HEAD no longer points to a branch—it points directly to a commit. This is called a “detached HEAD” state, and it can be visualized as the following:
...
8233cdd initial commit
```
1. You can git checkout to view the "added snaps" commit as follows:
```
$ git checkout 8a326b7
```

1. To continue developing, you need to get back to the “current” state of your project:
```
$ git checkout master
```
...
1. **Create:** Create a new branch called <branch>. This does not check out the new branch.
```
...
```
1. **Delete:** To delete a specify branch you can type
```
...
$ git branch -D <branch-name>
```
1. **Rename:** To rename current branch as branch-new you can type
```
...
1. **Create and Checkout** To create and checkout a branch
```
